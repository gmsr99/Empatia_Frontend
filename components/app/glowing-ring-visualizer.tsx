...
export function GlowingRingVisualizer({ stream, height = '300px', width = '300px' }: GlowingRingVisualizerProps) {
    ...
    useEffect(() => {
        ...
        const renderFrame = () => {
            ...
            drawOrganicRing(ctx, centerX, centerY, radius * 0.95, normalizedEnergy, phase, '#f3e8ff', 0.8);
            ...
            drawOrganicRing(ctx, centerX, centerY, radius * 1.02, normalizedEnergy * 0.9, phase + 2.5, '#22d3ee', 0.5);

            // Layer 4: Outer Violet (Ghost)
            drawOrganicRing(ctx, centerX, centerY, radius * 1.05, normalizedEnergy * 0.8, phase + 4, '#a855f7', 0.3);

            // Layer 5: Outer Cyan (Ghost) - widest
            drawOrganicRing(ctx, centerX, centerY, radius * 1.1, normalizedEnergy * 0.7, phase + 5.5, '#22d3ee', 0.2);
    ...
    if (!stream) {
        ...
        return (
            <div className="flex items-center justify-center text-brand-lilac/50 font-mono text-sm animate-pulse" style={{ height, width }}>
                Waiting for audio...
            </div>
        )
    }
    ...
    return (
        <div ref={containerRef} className="relative flex items-center justify-center" style={{ height, width }}>
            <canvas
                ref={canvasRef}
